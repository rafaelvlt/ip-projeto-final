...
class Game:
    ...
    def iniciar_novo_jogo(self):
        ...
        self.life_orb = Items(posicao=(300, 300), sheet_item=self.assets_itens["life_orb"],  tipo='life_orb',  grupos=(self.all_sprites, self.item_group))
        self.expShard = Items(posicao=(700, 500), sheet_item=self.assets_itens["exp_shard"], tipo='exp_shard', grupos=(self.all_sprites, self.item_group))
        self.bigShard = Items(posicao=(200, 400), sheet_item=self.assets_itens["big_shard"], tipo='big_shard', grupos=(self.all_sprites, self.item_group))
    ...
    def colisao(self):
        ...
        for inimigo in list(self.inimigos_grupo):  #iterar sobre cópia
            if inimigo.vida <= 0:
                # dropa item na posição do inimigo
                pos_drop = getattr(inimigo, "posicao", None)
                if pos_drop is not None:
                    pos = (pos_drop.x, pos_drop.y)
                else:
                    pos = inimigo.rect.center
                self._drop_item_aleatorio(pos)
