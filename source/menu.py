...
class Colaboradores:
    def __init__(self, game):
        self.game = game
        self.bg = pygame.image.load(join('assets', 'img', 'colaboradores.jpeg')).convert()
        self.font = pygame.font.Font(None, 48)
        self.input_rects = [
            pygame.Rect(400, 200 + i * 70, 400, 50) for i in range(5)
        ]  # até 5 colaboradores

        # Preenchido com seu nome e login
        self.nomes = [
            "Alvaro Miguel de Oliveira Lima (amol)",
            "",  # Espaço para outros colaboradores
            "",
            "",
            ""
        ]

        # Desativa entrada de texto, apenas visualização
        self.input_mode = False

        # Cores para os retângulos de destaque
        self.active_idx = 0
        self.active_color = pygame.Color('lightskyblue3')
        self.inactive_color = pygame.Color('gray15')
        self.colors = [self.inactive_color] * 5
        self.colors[self.active_idx] = self.active_color

    def handle_event(self, event):
        # Permite sair da tela com ESC
        if event.type == pygame.KEYDOWN:
    ...
    def draw(self, tela):
        tela.blit(self.bg, (0, 0))
        title = self.font.render("Colaboradores", True, (255, 255, 255))
        tela.blit(title, (largura_tela // 2 - title.get_width() // 2, 100))

        for i, rect in enumerate(self.input_rects):
            pygame.draw.rect(tela, self.colors[i], rect, 2)
            nome_surface = self.font.render(self.nomes[i], True, (255, 255, 255))
            tela.blit(nome_surface, (rect.x + 10, rect.y + 5))

        info_text = self.font.render("ESC para sair", True, (200, 200, 200))
        tela.blit(info_text, (largura_tela // 2 - info_text.get_width() // 2, altura_tela - 100))
